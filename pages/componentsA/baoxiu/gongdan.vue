		...
		<view class="body_gong" v-for="(o,index) in list" :key="index" >
			<view class="title"  @click="xq(o.id)">{{o.lx==1?"房屋报修":"公共报修"}} 
				<text class="zhaungtai" :class="o.status==1?'color2':o.status==2 ? 'color3' :o.status==3 ? 'color4':o.status==4 ? 'color5':'color1'">
					{{o.status===1 ? "待处理" : o.status==2?"维修中":o.status==3?"待评价":o.status==4?"已评价":"待处理"}}
				</text>
			</view>
...
export default {
		...
		methods: {
			gethome() {
				uni.$u.http.post('Repair/gdlist', {wxid:uni.getStorageSync('userData').uid,page:this.page,limit:10}).then(res => {
			...
			xq(id){
				uni.$u.route({
					url: "/pages/componentsA/baoxiu/details_gd?id="+id
				})
